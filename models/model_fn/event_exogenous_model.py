...
def da_rnn_fn(features, labels, mode, params):
    ...
    lstm1 = attention_based_lstm.attention_based_lstm(state_size, batch_size, truncated_backprop_length, keep_rate)
    exogenous_hidden_states = lstm1.run(Y, attention_hidden_states)
    ...
    event_hidden_states = lstm2.run(events)
    event_hidden_states = event_hidden_states[1:]

    # combine event_hidden_state and exogenous_hidden_states
    exogenous_hidden_states = tf.unstack(exogenous_hidden_states, axis=1)
